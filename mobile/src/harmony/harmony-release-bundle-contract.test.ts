...
describe('Harmony release bundle contract', () => {
  ...
  it('fails closed when source scanning or the DevEco linter does not run', () => {
    ...
    expect(nativeLinter).toContain('listNativeLintSources(')
    expect(nativeLinter).toContain('inspectCodeLinterResult(result, expectedSources)')
    expect(nativeLinter).toContain('process.exitCode = inspection.failures.length > 0 ? 1 : 0')
  })
})
